...
function generateUsers() {
	...
	// GENERATE USER INFORMATION
		...
		// RANDOM NAME (First and Last)
			...
			$rollGender = mt_rand(0,2);
		
			if($rollGender == 0)
			{
				$firstLine = MALE[array_rand(MALE)];
				// store the randomly selected name
				$firstRanName = strtolower($firstLine);
				// GENDER (3 choices - 0,1,2 )
				$gender = 0;
			}
			else if($rollGender == 1)
			{
				$firstLine = FEMALE[array_rand(FEMALE)];
				// store the randomly selected name
				$firstRanName = strtolower($firstLine);
				// GENDER (3 choices - 0,1,2 )
				$gender = 1;
			...
			{
				$firstLine = OTHER[array_rand(OTHER)];
				// store the randomly selected name
				$firstRanName = strtolower($firstLine);
				// GENDER (3 choices - 0,1,2 )
				$gender = 2;
		...
		// RANDOM LOGIN ID
			// randomize user id from name
			$userinfo = $firstRanName[0] . $lastRanName;
		
		// RANDOM EMAIL
			// pre-randomization (via name randomization above)
			// provider randomization
			$providers = array("@hotmail.com", "@gmail.com", "@yahoo.ca", "@live.ca");
			$email = strtolower($firstRanName) . strtolower($lastRanName) . $providers[mt_rand(0, count($providers) - 1)];
		...
		// RANDOM BIRTHDAY
			...
			$start = strtotime("1965-01-01");
			// end point of date range
			$end = strtotime("1999-10-12");
			// randomize within range
			$randomDate = mt_rand($start, $end);
			// format randomized date into a readable date string
			$birthday = date("Y-m-d", $randomDate);
			
		// RANDOM ENROLL DATE
			// start point of date range
			$start = strtotime("2012-10-12");
			// end point of date range
			$end = strtotime("2017-10-12");
			// randomize within range
			$randomDate = mt_rand($start, $end);
			// format randomized date into a readable date string
			$enrollDate = date("Y-m-d", $randomDate);

		// RANDOM LAST ACCESS
			// start point of date range
			$start = strtotime("2017-10-12 2:02");
			// end point of date range
			$end =  strtotime("2017-10-20 12:43");
			// randomize within range
			$randomDate = mt_rand($start, $end);
			// format randomized date into a readable date string
	...
	// GENERATE PROFILE DETAILS
		...
		// IMAGE (default: 0)
			$image = 0;
		// HEADLINE (text stuff)
			$headline = "What do you stand for?";
		// SELF DESCRIPTION (text stuff)
			$selfDescription = "Tell us a little about yourself...";
		// MATCH DESCRIPTION (text stuff)
			$matchDescription = "How would you describe your ideal match?";
		// RELATIONSHIP SOUGHT (5 choices)
			$relSought = mt_rand(0,4);
		// RELATIONSHIP STATUS (6 choices)
			$relStatus = mt_rand(0,5);
		// PREFERRED MIN AGE ()
			$prefMin = mt_rand(18,24);
		// PREFERRED MAX AGE
			$prefMax = mt_rand(24,40);
		// RELIGION SOUGHT
			$religionSought = mt_rand(0,9);
		...
		// HABIT
			$habit = mt_rand(0,3);
		// EXERCISE
			$exercise = mt_rand(0,3);
		// RESIDENCE TYPE
			$residenceType = mt_rand(0,6);
		// CAMPUS
			$campus = mt_rand(0,3);
