...
// DATABASE CONNECTION
	...
	$results = pg_prepare($connection, "find_user", 'SELECT id, password, first_name, last_name, email_address, account_type, birthday, enroll_date, last_access FROM users WHERE id = $1');

	$results = pg_prepare($connection, "select_id_pass", 'SELECT id, password, first_name, last_name, email_address, birthday, enroll_date, last_access FROM users WHERE id = $1 AND password = $2');
